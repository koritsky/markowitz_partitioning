...
class Partitioning(BinaryProblem):
    ...
    def mixed_matrix_generator(block_dim=None, block_mat=None):
        ...
        print(mixed_mat)
        print("\n")
        return block_mat, mixed_mat, p_mat
    ...
    @staticmethod
    def rand_sym_block_gen(block_dim: list, order=False):
        ...
        for dim in block_dim:
            current_dim += dim
            if order:
                block = np.triu(np.random.rand(dim, dim))
                for i in range(dim):
                    block += (dim - i) * np.eye(dim, k=i)
                block /= (dim+1)
            else:
                block = np.random.randn(dim, dim)

            # Make it symmetric
            block = (block + np.tril(block.T, -1))
                # if len(block) < 5:
                #     # Make each block ordered
                ...
                # else:
                #     # TODO: think about making it easier and fix it
                #     warnings.warn("Cant fix block with size > 4, because it uses exact_solver")
...
if __name__ == "__main__":
    ...
    print(Partitioning.rand_sym_block_gen([3, 4], order=True))